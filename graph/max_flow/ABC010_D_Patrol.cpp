// ABC010 D - Patrol
// https://atcoder.jp/contests/abc010/tasks/abc010_d
// 辺を切って到達不能にする最小コスト = 最小カット = 最大フロー
#include <bits/stdc++.h>
using namespace std;
struct MaxFlow{
...
int main(){
    int N,G,E;cin>>N>>G>>E;
    // super sink T connected from all girl nodes
    int S=0,T=N;
    MaxFlow mf(N+1);
    for(int i=0;i<G;i++){int g;cin>>g;g--;mf.add(g,T,INT_MAX);}
    for(int i=0;i<E;i++){int a,b;cin>>a>>b;a--;b--;mf.add(a,b,1);mf.add(b,a,1);}
